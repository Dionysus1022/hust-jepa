...
def _validate_benchmark_mode(task_suite, args) -> None:
    ...
    if mode not in {"libero", "libero_plus"}:
        raise ValueError("`benchmark_mode` must be either `libero` or `libero_plus`.")

    expected_count = VANILLA_LIBERO_TASK_COUNTS.get(args.task_suite_name)
    if mode == "libero" and expected_count is None:
        raise ValueError(
            f"`{args.task_suite_name}` is not a vanilla LIBERO suite. "
            "Use `--args.benchmark-mode libero_plus` for LIBERO-plus-only suites."
        )
    if mode == "libero" and expected_count is not None and task_suite.n_tasks != expected_count:
        raise ValueError(
            f"Expected vanilla LIBERO suite `{args.task_suite_name}` to have {expected_count} tasks, "
            f"but imported benchmark has {task_suite.n_tasks}. This looks like a LIBERO-plus benchmark. "
            "Use `--args.benchmark-mode libero_plus` for LIBERO-plus evaluation, or run with a vanilla "
            "LIBERO package/environment for standard LIBERO evaluation."
        )
