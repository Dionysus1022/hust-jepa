...
class VLA_JEPA(baseframework):
    ...
    def _encode_video_states(
        ...
        """Encode each temporal tubelet as an independent sample."""
        if input_videos.ndim != 5:
            raise ValueError(
                "Expected preprocessed videos with shape [B*V, T, C, H, W], "
                f"got {tuple(input_videos.shape)}"
            )
        expected_videos = batch_size * num_views
        if input_videos.shape[0] != expected_videos:
            raise ValueError(
                f"Expected {expected_videos} preprocessed videos, got {input_videos.shape[0]}"
            )
        if input_videos.shape[1] != num_frames:
            raise ValueError(
                f"Expected {num_frames} preprocessed frames, got {input_videos.shape[1]}"
            )
        expected_frames = int(self.config.framework.vj2_model.num_frames)
        if num_frames != expected_frames:
            raise ValueError(
                f"Expected {expected_frames} video frames to match the action prompt, got {num_frames}"
            )

        videos = input_videos.reshape(
            batch_size,
            num_views,
            *input_videos.shape[1:],
        )
        tubelets, dimensions = split_video_into_tubelets(
            videos,
            self.vj_encoder.config.tubelet_size,
        )
        with torch.no_grad():
            tubelet_embeddings = self.vj_encoder.get_vision_features(
                pixel_values_videos=tubelets
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            video_embeddings, tokens_per_frame = timed(
                "forward/vj_encoder_time",
                lambda: self._encode_video_states(
                    input_videos,
                    batch_size=B,
                    num_views=V,
                    num_frames=T,
            ...
            input_states = video_embeddings[:, :-tokens_per_frame, :]
