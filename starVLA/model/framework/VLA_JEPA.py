...
from starVLA.model.modules.dynamic_loss_weight import DynamicLossWeightController
...
class VLA_JEPA(baseframework):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        loss_cfg = self.config.trainer.get("dynamic_loss_weight", {})
        self.loss_controller = DynamicLossWeightController(
            enabled=loss_cfg.get("enabled", False),
            action_base_weight=loss_cfg.get("action_base_weight", 1.0),
            world_base_weight=loss_cfg.get("world_base_weight", 0.1),
            world_only_weight=loss_cfg.get("world_only_weight", 1.0),
            ema_decay=loss_cfg.get("ema_decay", 0.99),
            world_to_action_gain=loss_cfg.get("world_to_action_gain", 1.0),
            action_to_world_gain=loss_cfg.get("action_to_world_gain", 1.0),
            max_multiplier=loss_cfg.get("max_multiplier", 3.0),
            warmup_steps=loss_cfg.get("warmup_steps", 100),
    ...
    def _encode_video_states(self, batch_videos: np.ndarray) -> Tuple[torch.Tensor, int]:
        """Encode every temporal tubelet independently to prevent future leakage."""
        tubelet_size = self.vj_encoder.config.tubelet_size
        tubelets, (batch_size, num_views, num_steps) = split_video_into_tubelets(
            batch_videos,
            tubelet_size,
        )

        expected_frames = self.config.framework.vj2_model.num_frames
        actual_frames = batch_videos.shape[2]
        if actual_frames != expected_frames:
            raise ValueError(
                f"Expected {expected_frames} video frames to match the action prompt, "
                f"got {actual_frames}"
            )
        ...
        for tubelet in tubelets:
            ...
            if processed.ndim != 5 or processed.shape[1] != tubelet_size:
                raise ValueError(
                    "The video processor must preserve one encoder tubelet; expected "
                    f"[1, {tubelet_size}, C, H, W], got {tuple(processed.shape)}"
                )
            input_videos.append(processed.to(self.vj_encoder.device))
        input_videos = torch.cat(input_videos, dim=0)

        with torch.no_grad():
            tubelet_embeddings = self.vj_encoder.get_vision_features(
                pixel_values_videos=input_videos
            )

        return merge_multiview_tubelet_embeddings(
            tubelet_embeddings,
    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            # Step 2: JEPA Encoder
            video_embeddings, tokens_per_state = self._encode_video_states(batch_videos)
        
            # Step 3: VJ Predictor
            input_states = video_embeddings[:, :-tokens_per_state, :]
            gt_states = video_embeddings[:, tokens_per_state:, :]
        ...
        if "action" not in examples[0]:
            return self.loss_controller.world_only(teacher_forcing_wm_loss)
        ...
        return self.loss_controller(action_loss, teacher_forcing_wm_loss)
